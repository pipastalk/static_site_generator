...
def split_nodes_link(old_nodes): #wrapper function
    return split_nodes_special(old_nodes, TextType.LINK)

def split_nodes_image(old_nodes): #wrapper function
    return split_nodes_special(old_nodes, TextType.IMAGE)
            ...
            case _:
                ...
                if not matches:
                    ...
                    if suffix_node is not None:
                        split_suffix_nodes = split_nodes_special([suffix_node], text_type)
...
def text_to_text_nodes(text):
    ...
    for tag in TextType:
        ...
        if tag.markdown_open == tag.markdown_close:
            nodes = split_nodes_delimiter(nodes, tag.markdown_open, tag)
        else:
...
def markdown_to_html_node(markdown):
    ...
    for block in blocks:
        ...
        if block_type == BlockType.CODEBLOCK:
            ...
            pre_wrapper.children.append(text_node_to_html_node(codeblock_text_node))
            line_node = pre_wrapper
        elif block_type in [BlockType.ORDERED_LIST_ITEM, BlockType.UNORDERED_LIST_ITEM]:
            line_node = list_block_to_html_nodes(block, block_type)
        else:
            line_node = block_to_html_nodes(block, block_type)
...
def block_to_html_nodes(block, block_type):
    ...
    if block_type not in [BlockType.UNORDERED_LIST_ITEM, BlockType.ORDERED_LIST_ITEM] and block_type.markdown is not None:
        if block_type is BlockType.CODE:
            block = block[len(block_type.markdown):-len((block_type.markdown))]
        else:
    ...
    for i in range(len(text_nodes)):
        ...
        leaf_node = text_node_to_html_node(text_nodes[i])
...
def list_block_to_html_nodes(block, block_type):
    ...
    for i in range(len(block_lines)):
        ...
        if block_type == BlockType.ORDERED_LIST_ITEM:
            ...
            list_item_nodes = block_to_html_nodes(block=snipet, block_type=BlockType.ORDERED_LIST_ITEM)
        elif block_type == BlockType.UNORDERED_LIST_ITEM:
            ...
            snipet = re.sub(delimiter, "", snipet)
            list_item_nodes = block_to_html_nodes(block=snipet, block_type=BlockType.UNORDERED_LIST_ITEM)
        else:
            raise ValueError(f"Unsupported BlockType for list_block_to_html_nodes: {block_type!r}")
...
def generate_page(from_path, template_path, dest_path, base_path="/"):
    ...
    source_html = markdown_to_html_node(source_markdown).to_html()
    source_title = extract_title(source_markdown)
    final_html = finalize_html(template_html, source_title, source_html, base_path)
...
def finalize_html(template_html, source_title, source_html, base_path="/"):
    ...
    return final_html
def generate_page_recursive(template_path, directory, dest_path, base_path="/"):
